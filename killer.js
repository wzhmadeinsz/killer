...
var actiondeadword=document.getElementById("judge_action-deadword");
var actionturnword=document.getElementById("judge_action-turnword");
var actionnum=1;
var judge=document.getElementsByClassName("judgebook");
var dayofjudge=document.getElementsByClassName("judge_day");
var result=document.getElementsByClassName("result");
var resultboxvictory=document.getElementsByClassName("result_box-victory");
...
var killman_detail=document.getElementsByClassName("killman_detail");
var judgedetaildis=document.getElementsByClassName("judge_detailedis");
var playagain=document.getElementById("result_footer-again");
var judgebookdetail=document.getElementsByClassName("judge_checkiden-box");
var judge_checkiden=document.getElementsByClassName("judge_checkiden");
judge_checkiden[0].style.display='none';
var judge_checkidenstart=document.getElementsByClassName("judge_checkiden-start");
var player=[];
var killer=[];
var cunmin=[];
var alliden=[];
	...
	for(var i=0;i<player.length;i++)
	{
		...
		{	
			...
			if(alliden[randnum]!=null)
				...
				judgebookdetail[0].innerHTML=judgebookdetail[0].innerHTML+"<div class='judge_checkiden-detail'>"+player[i].num+"<br>"+"状态:"+player[i].alive+"<br>身份:"+player[i].iden+"<div>";
...
idende.onclick=function(){
		if(idende.innerHTML=="法官查看"){
			idensend.style.display="none";
			judge_checkiden[0].style.display='block';
		}
		if(i==(player.length))
			{idensend.style.display='none';}
...
actionvote.onclick=function(){
	if(actionnum==4){
		actionvote.style.background='grey';
		alert("请投票");
		actionnum=1;
		killman[0].style.display='block';
		judge[0].style.display='none';
		resultboxdetadis[0].innerHTML=resultboxdetadis[0].innerHTML+"白天:";
		judgedetaildis[0].innerHTML=judgedetaildis[0].innerHTML+"白天:";
		numofday++;
		dayofjudge[0].innerHTML="第 "+numofday+" 天";
		actionvote.style.background='#24a7c6';
		actionkill.style.background='#24a7c6';
		actiondeadword.style.background='#24a7c6';
		actionturnword.style.background='#24a7c6';
		killman_detail[0].innerHTML='';
		for(var kkk=0;kkk<player.length;kkk++)
		{	
			killman_detail[0].innerHTML=killman_detail[0].innerHTML+"<div class='judge_checkiden-detail'>"+player[kkk].num+"<br>"+"状态:"+player[kkk].alive+"<br><div>";}
	}
	else{
		alert("请按游戏顺序进行");
	}
...
actionkill.onclick=function(){
	if(actionnum==1){
		actionkill.style.background='grey';
		alert("请杀手杀人");
		actionnum++;
		killman[0].style.display='block';
		judge[0].style.display='none';
		resultboxdetadis[0].innerHTML=resultboxdetadis[0].innerHTML+"第"+numofday+"天<br>晚上:";
		judgedetaildis[0].innerHTML=judgedetaildis[0].innerHTML+"第"+numofday+"天<br>晚上:";
		killman_detail[0].innerHTML='';
		for(var kkk=0;kkk<player.length;kkk++)
		{	
			killman_detail[0].innerHTML=killman_detail[0].innerHTML+"<div class='judge_checkiden-detail'>"+player[kkk].num+"<br>"+"状态:"+player[kkk].alive+"<br><div>";}
...
actiondeadword.onclick=function(){
	if(actionnum==2){
		actiondeadword.style.background='grey';
		alert("请死者发表遗言");
		actionnum++;
	}
	else{
...
actionturnword.onclick=function(){
	if(actionnum==3){
		actionturnword.style.background='grey';
		alert("请玩家按顺序发言");
		actionnum++;
	}
	else{
...
judgedetaildis[0].innerHTML=judgedetaildis[0].innerHTML+numofdeath+"号玩家出局，真实身份为"+player[numofdeath-1].iden+"<br>";
...
judge_checkidenstart[0].onclick=function(){
judge_checkiden[0].style.display='none';
judge[0].style.display='block';
}
