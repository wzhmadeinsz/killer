...
playingsend.onclick=function(){
	...
	allien=killer.concat(cunmin);
	//Math.floor(Math.random()*allien.length);//0--length-1
	for(var i=0;i<player.length;i++)
	{
		for(var k=0;player[i].iden=="玩家";k++)
		{	
			var randnum=Math.floor(Math.random()*allien.length);
			if(allien[randnum]!=null)
				{player[i].iden=allien[randnum];
						allien[randnum]=null;
					}
		}
	}
...
dayofdetail.innerHTML=/*dayofdetail.innerHTML+" "+*/numofdeath+"号玩家出局,他的身份是"+player[numofdeath-1].iden;
}
...
killman[0].style.display='block';
killnum.value='';
//numofdeath=killman.value;
//dayofdetail.innerHTML=numofdeath+"号玩家被杀";
}